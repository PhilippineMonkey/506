...
\item 表面热交换（对流）：
\begin{equation}
-k \left.\frac{\partial T}{\partial r}\right|_{r=R} = h(T_s - T_{env})
\end{equation}
\end{itemize}
求解得：\[ T(r, t)=T_{env}+\sum_{n=1}^{\infty} C_{n} \frac{\sin \left(\lambda_{n} r\right)}{r} e^{-\alpha \lambda_{n}^{2} t}\]
其中\[ C_n = \frac{2 (T_{0}-T_{env}) [\sin(\lambda_n R) - \lambda_n R \cos(\lambda_n R)]}{\lambda_n^2 R [1 - \cos(2\lambda_n R)/2]}\]
\par 大意：对上述方程带入参数，画出温度随半径和时间变化的图，待与后文仿真结果比较。
